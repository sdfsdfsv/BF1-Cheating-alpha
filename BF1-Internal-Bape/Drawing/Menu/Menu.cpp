...
namespace Menu
{
	...
	void ApplyStyle() {
		...
		style->Colors[ImGuiCol_Text] = ImVec4(0.80f, 0.80f, 0.83f, 1.00f);//gray
	...
	void Draw()
	{
...

		{
			...
			/* "Tab" buttons */
			if (ImGui::Button("Aim", ImVec2(126, 25))) 
				m_CurrentTab = 1; 
			ImGui::SameLine(0.0, 4.0f);
			if (ImGui::Button("Visuals", ImVec2(126, 25))) 
				m_CurrentTab = 2;
			ImGui::SameLine(0.0, 4.0f);
			if (ImGui::Button("Misc.", ImVec2(126, 25))) 
				m_CurrentTab = 3;
			ImGui::SameLine(0.0, 4.0f);
			if (ImGui::Button("Colors", ImVec2(126, 25))) 
				m_CurrentTab = 4;
			...
			switch (m_CurrentTab) {
			case 1: /* aimbot */
				ImGui::Checkbox("自瞄", &global->aim_enabled);
				ImGui::SameLine();
				ImGui::Text(" (Disable raw input)原始输入");
				break;
			case 2: /* visuals */
